...
for x in range(prediction_days, len(scaled_data)):
    x_train.append(scaled_data[x - prediction_days:x, :])  # Using all OHLC values
    y_train.append(scaled_data[x, 3])  # Considering only the Close price
...
models_inputs = models_inputs.reshape(-1, 4)
...
for x in range(prediction_days, len(models_inputs)):
    x_test.append(models_inputs[x - prediction_days:x, :])
...
# Predicting Next Day
real_data = [models_inputs[len(models_inputs) + 1 - prediction_days:len(models_inputs) + 1, :]]
